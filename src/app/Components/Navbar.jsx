...
const courseMenu = [
  {
    ...
    items: [
      ...
      {
        label: "Class 10 — Java",
        href: "/courses/icse-class-10-java-dehradun",
      },
    ],
  },
  {
    ...
    items: [
      {
        label: "Class 11 — Python",
        href: "/courses/cbse-class-11-python-dehradun",
      },
      {
        label: "Class 12 — Python",
        href: "/courses/cbse-class-12-python-dehradun",
      },
    ],
  },
  {
    ...
    items: [
      {
        label: "Java · Python · C · C++ · C# · PHP",
        href: "/courses/java-python-programming-dehradun",
      },
      {
        label: "Web Development",
        href: "/courses/web-development-course-dehradun",
      },
    ],
  },
  {
...
function MobileLink({ href, icon, label, onClick }) {
  return (
    <li
      className="flex items-center p-3 rounded-lg hover:bg-[#0C1233] transition"
      role="none"
    >
...
export default function Navbar() {
  ...
  const handleMouseLeave = () => {
    leaveTimer.current = setTimeout(() => setOpenMenu(null), 120);
  };
  ...
  const handlePhoneClick = () => {
    if (typeof window !== "undefined") {
      ...
      window.dataLayer.push({
        event: "phone_click",
        phone_number: "9837218345",
        location: "navbar",
      });
      if (window.gtag) {
        window.gtag("event", "conversion", {
          send_to: "AW-16549958925/PHONE_LABEL",
        });
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center">
            <Link
              href="/"
              aria-label="Home - CodewareIT Best Computer Coding Institute Dehradun"
            >
              <Image
                src="/logo.png"
                width={80}
                height={90}
                alt="CodewareIT — Best Computer Coding Institute Dehradun"
                priority
                className="pt-1"
              />
            </Link>
          ...
          >
            <li role="none">
              ...
              <Link
                href="/about-best-computer-coding-institute-in-dehradun"
            ...
            >
              <button
                className="flex items-center gap-1 text-sm font-medium hover:text-blue-400 transition-colors py-6"
                aria-haspopup="true"
                aria-expanded={openMenu === "Courses"}
              >
                Courses
                <FaChevronDown
                  className={`text-xs transition-transform duration-200 ${openMenu === "Courses" ? "rotate-180" : ""}`}
                />
              </button>
                  ...
                  <div className="bg-[#0A1C55] border border-white/10 rounded-2xl shadow-2xl p-5 mt-1 w-[680px] grid grid-cols-2 gap-4">
                    {courseMenu.map((cat) => (
                      <div key={cat.label} className="space-y-1">
                        <div
                          className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-2 px-3"
                        ...
                        {cat.items.map((item) => (
                          <Link
                            ...
                            className="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-white/10 transition-all duration-150"
                          >
                            {item.label}
                          </Link>
                        ))}
                    ...
                    <div className="col-span-2 border-t border-white/10 pt-3 flex gap-4">
                      ...
                      <Link
                        href="/courses-best-computer-coding-institute-in-dehradun"
                        onClick={() => setOpenMenu(null)}
            ...
            <li role="none">
              <Link
                href="/internship-best-computer-coding-institute-in-dehradun"
                className="hover:text-blue-400 transition-colors text-sm font-medium"
              >
                Internship
              </Link>
            ...
            <li role="none">
              <Link
                href="/students-best-computer-coding-institute-in-dehradun"
                className="hover:text-blue-400 transition-colors text-sm font-medium"
              >
                Students
              </Link>
      ...
      {isOpen && (
        <div
          id="mobile-menu"
          className="fixed top-[5rem] left-0 w-full h-screen bg-[#040A26] z-50 p-4 md:hidden overflow-y-auto"
          role="menu"
        >
          <ul className="text-white space-y-2">
            ...
            <li className="rounded-lg overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-[#0C1233] transition"
              >
                <div className="flex items-center gap-3">
                ...
                <FaChevronDown
                  className={`text-gray-400 text-sm transition-transform duration-200 ${mobileCoursesOpen ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <div className="bg-[#0C1233] rounded-b-lg ml-2 mr-2 mb-1">
                  {courseMenu.map((cat) => (
                    <div key={cat.label}>
                      <button
                        onClick={() =>
                          setMobileCategoryOpen(
                            mobileCategoryOpen === cat.label ? null : cat.label,
                          )
                        }
                      ...
                      >
                        ...
                        <FaChevronDown
                          className={`text-gray-500 text-xs transition-transform duration-200 ${mobileCategoryOpen === cat.label ? "rotate-180" : ""}`}
                        />
                      </button>
                        ...
                        <div className="pl-6 pb-2 space-y-1">
                          {cat.items.map((item) => (
                            <Link
                              ...
                              href={item.href}
                              onClick={toggleMobileMenu}
                              className="block py-2 px-3 text-sm text-gray-300 hover:text-white rounded-lg hover:bg-white/10 transition"
                            >
                              {item.label}
                            </Link>
                          ))}
                  ...
                  <div className="px-4 pb-3 pt-2 border-t border-white/10 space-y-1">
                    <Link
                      href="/courses-best-computer-coding-institute-in-dehradun"
                      onClick={toggleMobileMenu}
                      className="block text-xs text-blue-400 hover:text-blue-300 py-1"
                    >
                      → All Coding Courses
                    </Link>
                    <Link
